...
impl<TS: KeycloakTokenSupplier> KeycloakAdmin<TS> {
    ...
    pub async fn realm_clients_with_client_uuid_evaluate_scopes_generate_example_saml_response_get(
        &self,
        realm: &str,
        client_uuid: &str,
        audience: Option<String>,
        scope: Option<String>,
        user_id: Option<String>,
    ) -> Result<SamlExampleResponse, KeycloakError> {
        let realm = p(realm);
        let client_uuid = p(client_uuid);
        let mut builder = self
            .client
            .get(format!(
                "{}/admin/realms/{realm}/clients/{client_uuid}/evaluate-scopes/generate-example-saml-response",
                self.url
            ))
            .bearer_auth(self.token_supplier.get(&self.url).await?);
        if let Some(v) = audience {
